...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { termsOfService } = getLegalContent(locale);

  return { title: termsOfService.title, description: termsOfService.description };
}

export default async function TermsPage({ params }: Props) {
  const { locale } = await params;

  return <LegalArticle doc={getLegalContent(locale).termsOfService} />;
}
